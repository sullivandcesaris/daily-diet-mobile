...
export const TrueRadio = styled.TouchableOpacity<RadioProps>`
  ...
  ${({ theme, selected }) =>
    selected === true ? "border: 1px solid " + theme.COLORS.GREEN_700 : ""};
  flex: 1;
  padding: 16px;
  border-radius: 8px;
`;
...
export const FalseRadio = styled.TouchableOpacity<RadioProps>`
  ...
  ${({ theme, selected }) =>
    selected === false ? "border: 1px solid " + theme.COLORS.RED_700 : ""};
  flex: 1;
  padding: 16px;
  border-radius: 8px;
`;
...
export const OptionCircle = styled.View<CircleProps>`
  ...
  background-color: ${({ theme, type }) =>
    type === true ? theme.COLORS.GREEN_700 : theme.COLORS.RED_700};
`;
